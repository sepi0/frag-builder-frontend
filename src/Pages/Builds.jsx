...
import { faPaperPlane, faShoppingCart, faTimes, faArrowLeft } from "@fortawesome/free-solid-svg-icons";
			...
			<div className="px-6 py-4">
				<div className="flex flex-col justify-between">
					<div className="font-cairo">
						<h2 className="font-archivo font-bold text-4xl text-carbon-500 mb-2">
							{props.model}
						</h2>
					...
					<div className={"w-3 self-center"}>
						<Button color={"bubblegumRed-500"} onClick={props.onClick}>
...
export default class Builds extends React.Component {
	...
	toggleCart = () => {
		this.setState({ cartVisible: !this.state.cartVisible }, console.log(this.state.inCart))
	}
	...
	cartButton = () => {
		if (!this.state.cartVisible) {
			return (
				<Button color={"bubblegumRed-500"} onClick={this.toggleCart}>
					Do košíka
					<FontAwesomeIcon className={"mx-2 self-center"} icon={faShoppingCart}/>
				</Button>
			)
		} else {
			return (
				<Button color={"carbon-500"} onClick={this.toggleCart}>
	...
	sendOrder = async (event) => {
        ...
        const json = JSON.stringify({
            orderId: orderId,
            components: this.state.inCart.map(item => item).toString(),
